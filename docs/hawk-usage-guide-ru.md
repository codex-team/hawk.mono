...
Hawk — это облачный open-source трекер ошибок, разработанный в России. Он позволяет отслеживать ошибки в веб-приложениях, API и мобильных сервисах. В этой статье мы подробно рассмотрим всю функциональность, которая сегодня доступна пользователям Хоука — от регистрации до продвинутых SDK-функций и планов на будущее.
...
2. Организация рабочего пространства
    - Воркспейсы
        ...
        - Платежи
            - Прикрепление карты
            - Разовые платежи
            - Включение/отключение авто-платежей
            - Оплата по счету
            - История операций
        - Покинуть воркспейс
...
5. События (ошибки)
    - Информация о событии
        ...
        - Исполнитель
        - Stack Trace
        - Addons – данные, собранные определенным SDK
        - Context - ручное прикрепление любых данных к событию
...
На текущий момент Хоук поддерживает вход только по email. После регистрации пароль приходит на указаннную почту. В дальнейшем его можно изменить в настройках аккаунта.
...
* **Системные уведомления**: можно включить или отключить email-уведомления о событиях и лимитах.
...
Рабочее пространство (воркспейс) — это логическая единица, в рамках которой ведется работа над проектами (разными интеграциями). Например, воркспейс может представлять собой компанию или команду, а проекты — frontend, backend, внутренние сервисы. Каждый пользователь может создать несколько воркспейсов.
...
В команду можно приглашать участников по email или ссылке. Существует два уровня доступа:

* **Администратор**: может управлять платежами и назначать новых администраторов.
* **Обычный участник**: может просматривать проекты и события, приглашать новых участников, назначать исполнителей, стаавить отметки о исправленных/игнорируемых ошибках.

**Важно**: если в воркспейсе только один администратор, он не сможет покинуть команду, пока не назначит другого администратора.

### Покинуть воркспейс

Пользователь может покинуть воркспейс, если он не является единственным администратором.
...
При оплате картой разово, план будт активирован на 1 месяц, после чего будет необходимо оплатить его снова.
...
Для более крупных тарифов компаниям иногда удобнее проводить оплату по счету. Для выставления счета необходимо обратиться в поддержку по почте [team@hawk.so](mailto:team@hawk.so) или в [Telegram](https://t.me/hawk_tracker) и прислать данные о компании, воркспейсе и тарифном плане. 

Активация плана происходит в течение 48 часов после оплаты счета. Закрывающие документы будут присланы по запросу. 

##### История операций

В истории операций можно посмотреть все платежи, связанные с воркспейсом.
...
Для перехода к настройкам проекта нужно переключиться на нужный проект с помощью левой колонки с последними ошибками, а затем нажать на название проекта в верхней части страницы.
...
* **Фильтры**: по тексту ошибки, включить/исключить ключевые слова;

Примеры сценариев:

- Если произошла **новая** ошибка со словом "Payment", отправить уведомление на Email
- Если ошибка на бэкенде выпала больше 500 раз за 5 минут, отправить уведомление прямо в чат с разработчиками в Telegram
...
Для настройки группировки по паттернам необходимо перейти в настройки проекта и в разделе "Группировка событий" добавить нужные паттерны, указав регулярное выражение.

Пример регулярного выражения для группировки ошибок с разными тайтлами, но вызванных одной проблемой:
...
В списоке событый работает поиск, который ищет совпадения по:
...
* Дату первого появлениия
* Дату текщуего просматриваемого повторения
...
События можно обогощать собственными данными, прикрепляя их вручную двумя способами: 
...
Такие ошибки будут выделяться в списке событий. Это поможет вам быстрее найти их и обратить на них внимание.
...
Актуальные характиристики и стоимость тарифов опублкованы на сайте Хоука: [https://hawk-tracker.ru](https://hawk-tracker.ru?utm_source=big-guide)
...
Все способы подходят как для физических, так и для юридических лиц. Подробне в разделе "Платежи".
...
- Возможность доработки функицональности Хоука по запросу, влияние на Roadmap, заказ новых SDK.
...
- В случае отстутствия оплаты, 3 дня воркспейс остается незаблокированным. Если оплата не была произведена, воркспейс блокируется.
...
- **Восстановление**: события снова начинают приниматься либо при переходе на следующий тариф, либо при повторой оплате текущего тарифа (обнулении счетчика и обновлении даты платежного периода), либо в начале следующего месяца (для бесплатного плана)
...
- **Исправление ошибок**: конечно, главное — воремя исправлять самые критичные ошибки.
...
Для всех клиентских (JavaScript, TypeScript, React, Vue, и т.д.) рекомендуется настроить отправку Source Maps. Так при просмотре информации об ошибке, вы сможете увидеть исходный код и названия функций.

#### Webpack Plugin

Плагин позволяет отправлять релизы с Source Maps и коммитами при сборке проекта. 
...
У Хоука есть специальный Action для GitHub Actions, который позволяет отправлять коммиты для релиза: [codex-team/hawk.releases@v1](https://github.com/marketplace/actions/hawk-release-sender)
...
Некторые SDK Хоука позволяют отключить глобальный отлов ошибок. Например, когда у них есть свой механизм отлова ошибок.
...
// Эксземпляр кэтчера можно сохранить и использовать в виде синглтона
const hawk = new HawkCatcher({
...
Можно передавать уникальный контекст прямо при вызове `hawk.send()`.
...
- Соотнести ошбику с конкретной версией приложения
...
Некоторые SDK позволяют указать уровень критичности события, которые должны отлавливатьcя. Это позволяет исключать неважные события из отправки.
...
Некторые SDK поддерживают интеграцию с логгерами. Это позволяет встроить Хоук в уже существующую логирующую систему.
...
Исходный код всех частей Хоука открыт и доступен на GitHub. Можно развернуть Хоук на своём сервере или VPS. Для удобства все сервисы собраны в моно-репозитории [Hawk Mono](https://github.com/codex-team/hawk.mono). Инструкицю по развертыванию можно найти там же.
